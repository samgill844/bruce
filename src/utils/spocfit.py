def main():
    ...
    class Window():
        ...
        def load_data(self, filenames):
            ...
            for i in range(len(filenames)):
                ...
                if 'KSPSAP_FLUX' in data_.colnames:
                    data_ = data_[data_['QUALITY']==0]
                    data_['SAP_FLUX'] = data_['KSPSAP_FLUX']
                    data_['PDCSAP_FLUX'] = data_['KSPSAP_FLUX']
                    data_['PDCSAP_FLUX_ERR'] = data_['KSPSAP_FLUX_ERR']
                if 'DET_FLUX' in data_.colnames:
                    data_ = data_[data_['QUALITY']==0]
                    data_['SAP_FLUX'] = data_['DET_FLUX']
                    data_['PDCSAP_FLUX'] = data_['DET_FLUX']
                    data_['PDCSAP_FLUX_ERR'] = data_['DET_FLUX_ERR']
            ...
            # Now filter the data
            self.data = self.data[~np.isinf(self.data['TIME']) & ~np.isnan(self.data['TIME']) & ~np.isinf(self.data['SAP_FLUX']) & ~np.isnan(self.data['SAP_FLUX']) & (self.data['SAP_FLUX'] > 0.1) & (self.data['PDCSAP_FLUX'] > 0.1)]
        ...
        def draw_data(self,):
            ...
            for i in range(len(self.sector_info)):
                ...
                try : text = pg.TextItem(text = 'Sector {:02}'.format(self.sector_info[i]['sector']))
                except : text = -1
    ...
    if (len(sys.argv)==2) and (sys.argv[1][-5:]!='.fits'):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            os.chdir(tmpdirname)
            os.system('spocget {:} --lc --download'.format(sys.argv[1]))
            files = np.sort(glob.glob('*.fits'))
            if len(files)==0 : 
                print('No SPOC files found, check if they have SPOC data!')
                exit()
            app = Window(title='TIC-{:}'.format(sys.argv[1]))
            app.load_data(files)
        os.chdir(home)
    else:
        try : tic = fits.open(sys.argv[1])[0].header['TICID']
        except : tic =1
        app = Window(title='TIC-{:}'.format(tic))
